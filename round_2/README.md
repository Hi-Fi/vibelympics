...
### Optional dependency
- For more accurate alignment when terminal output includes emojis or wide Unicode characters, install `wcwidth`:
...
- stdin handling: the auditor now preserves piped stdin when using `-` (useful for CI pipelines that `cat package-lock.json | npm_auditor -`).
- Display alignment: an optional dependency `wcwidth` is included in `requirements.txt` to improve table alignment when emojis or wide Unicode characters appear. Installing the requirements and rebuilding the Docker image will make the runtime output more accurate.
- Banner / Branding: the CLI banner is now generated by `lib.formatters.print_banner()` and prints a compact two-line box; the renderer is display-aware so ANSI color codes and emoji do not misalign the box.
- Docker: the `Dockerfile` was updated to install `requirements.txt` into the build stage so optional packages like `wcwidth` can be present at runtime. Rebuild the image to pick up this change.
- Tests: a unit test `tests/test_formatters.py` was added to validate table/header alignment. Run it with `pytest` (see commands below).
...
**Formatter alignment test**

We added a focused unit test for the output formatter that asserts header and separator alignment (`tests/test_formatters.py`). This test is useful after making changes to `lib/formatters.py` or when you install `wcwidth`.

Run the formatter test (recommended to install `wcwidth` first):

```bash
cd round_2
python3 -m pip install -r requirements.txt
# run only the formatter test
python3 -m pytest -q tests/test_formatters.py
```

If you don't use `pytest`, you can run the test script directly:

```bash
cd round_2
PYTHONPATH=. python3 tests/test_formatters.py
```
